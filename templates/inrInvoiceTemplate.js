...
function renderInrInvoiceHtml(data = {}) {
  ...
  const productRowsHtml = productList.map(p => {
    ...
    return `<tr class="product-row">
      <td class="text-center">${exRate > 0 ? exRate.toFixed(2) : ''}</td>
      <td class="text-center" style="border-left: 2px solid #ffffff;">${packages}</td>
      <td class="text-center">${escapeHtml(p.productName || 'CERAMIC SANITARY WARE')}</td>
      <td class="text-center">${qty}</td>
      <td class="text-center">${escapeHtml(p.quantityUnit || 'SET')}</td>
      <td class="text-center">${usdPrice.toFixed(2)}</td>
      <td class="text-center">${inrPrice.toFixed(2)}</td>
      <td class="text-center">${lineInrAmt}</td>
  ...
  // Base64-encode logo, signature, and footer images (using uploaded images if available, otherwise default assets)
  ...
  html = html
    ...
    .replace('{{TOTAL_INR}}', totalInrAmount)
